...
state_ptr(ptr2)
{
    if (ctrl_ptr == nullptr || state_ptr ==nullptr) {
        std::cerr << "错误：传入的结构体指针为空！" << std::endl;
        exit(1);  // 终止程序（也可抛异常，新手先简化）
    }
...
void Sctrl::transmit(doubleaxis_ctrl* data1, doubleaxis_state* data2) {
    while (true) {
        ...
        // std::cout << std::bitset<16>(data2.state) << '\n';
        std::cout << data1[1].control << std::endl;
        std::cout << data2[1].state << std::endl;
        data2[1].state+=1;
